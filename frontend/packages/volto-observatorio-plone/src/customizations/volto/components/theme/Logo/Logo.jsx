/**
 * Logo — classe `observatorio-logo` para limitar tamanho via tema.
 */
...
const Logo = () => {
  ...
  return (
    <Link to={navRootPath} aria-label={intl.formatMessage(messages.home)}>
      <Image
        className="ui image observatorio-logo"
        style={{
          width: 'var(--obs-logo-max-width, 220px)',
          height: 'var(--obs-logo-max-height, 80px)',
          maxWidth: 'var(--obs-logo-max-width, 220px)',
          maxHeight: 'var(--obs-logo-max-height, 80px)',
          objectFit: 'contain',
        }}
        src={
