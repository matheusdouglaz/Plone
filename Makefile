...
STACK_NAME=observatorio-plone-example-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://observatorio-plone.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t matheusdouglaz/observatorio-plone-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t matheusdouglaz/observatorio-plone-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name observatorio-plone-frontend-acceptance --link observatorio-plone-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d matheusdouglaz/observatorio-plone-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name observatorio-plone-backend-acceptance -d matheusdouglaz/observatorio-plone-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop observatorio-plone-frontend-acceptance
	@docker stop observatorio-plone-backend-acceptance
